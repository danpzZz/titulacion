...
export class EnrollmentDetailListComponent implements OnInit {
    ...
    selectItem(item: EnrollmentDetailModel): void {
        ...
        if (this.canModify()) {
            ...
            if (isRegistered || isRequested) {
                actions.push({
                    label: 'Aprobar', icon: CustomIcons.CHECK_SOLID,
            ...
            if (isApproved) {
                actions.push({
                    label: 'Matricular', icon: CustomIcons.BOOK_SOLID,
            ...
            if (isRegistered || isRequested || isApproved) {
                actions.push({
                    label: 'Rechazar', icon: CustomIcons.CIRCLE_XMARK_SOLID,
            ...
            if (isApproved || isEnrolled) {
                actions.push({
                    label: 'Anular', icon: CustomIcons.BAN_SOLID,
            ...
            if (isRegistered || isRejected || isRevoked) {
                actions.push({
                    label: 'Eliminar', icon: CustomIcons.TRASH_CAN_SOLID,
    ...
    goToCreate(): void {
        this.router.navigate([
            SECRETARY_ROUTES.enrollment.form.absoluteFn(this.enrollmentId(), 'new')
        ]);
    }
    goToEdit(id: string): void {
        this.router.navigate([
            SECRETARY_ROUTES.enrollment.form.absoluteFn(this.enrollmentId(), id)
        ]);
    }
}
